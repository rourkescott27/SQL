...
FROM 'D:\SQL\external_data\pls_fy2014_pupld14a.csv'
...
FROM 'D:\SQL\external_data\pls_fy2009_pupld09a.csv'
WITH (FORMAT CSV, HEADER);

-- 8.3 
--- Using the count() function to verify the total row count of each table
SELECT count(*)
...
-- 8.4
-- Using the count() aggregate function to verify the number of values in a coloum
SELECT count(salaries)
FROM pls_fy2014_pupld14a;

-- 8.5
--- Using the count() aggregate function to verify DISTINCT values in a column
...
-- 8.6
--- Using max() & min() aggregate functions to contrast most and fewest visits
...
-- 8.8
--- Using GROUP BY for two columns
SELECT city, stabr
FROM pls_fy2014_pupld14a
GROUP BY city, stabr
ORDER BY city, stabr;
...
-- 8.9
--- Using GROUP BY with the count() aggregate function
SELECT stabr, count(*)
FROM pls_fy2014_pupld14a
GROUP BY stabr
ORDER BY count(*) DESC;
...
-- 8.10
--- Using GROUP BY and count() to determine location changes 
...
-- 8.11
--- Using the sum() aggregate function to see total visits to libraries in 2009 & 2014
...
-- 8.12
--- Better contrasting the library visits using the sum() aggregate function and joining the visits per year 
SELECT sum(pls14.visits) AS visits_2014,
       sum(pls09.visits) AS visits_2009
FROM pls_fy2014_pupld14a pls14 JOIN pls_fy2009_pupld09a pls09
ON pls14.fscskey = pls09.fscskey
WHERE pls14.visits >= 0 AND pls09.visits >= 0; -- For more accurate data 

--* 8.13
--- Contrasting library visits between 2009 & 2014 as well as calculating the percentage change between said years 
SELECT pls14.stabr,
...
--* 8.14
--- Using a HAVING clause to filter the results of an aggregate query for states with more than 50 million people
SELECT pls14.stabr,
       sum(pls14.visits) AS visits_2014,
       sum(pls09.visits) AS visits_2009,
       round( (CAST(sum(pls14.visits) AS decimal(10,1)) - sum(pls09.visits)) /
